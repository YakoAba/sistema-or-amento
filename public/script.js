...
function calcularTotalProduto() {
  ...
  produtos.forEach((produto) => {
    const quantidade =
      produto.querySelector('input[placeholder="Quantidade"]').value || 0;
    const valorUnitario =
      produto.querySelector('input[placeholder="Valor Unitário"]').value || 0;
...
function exibirCamposPrazos() {
  document
    .getElementById("prazoFabricacaoCheck")
    .addEventListener("change", function () {
      document.getElementById("prazoFabricacao").style.display = this.checked
        ? "block"
        : "none";
    });

  document
    .getElementById("prazoEntregaCheck")
    .addEventListener("change", function () {
      document.getElementById("prazoEntrega").style.display = this.checked
        ? "block"
        : "none";
    });

  document
    .getElementById("observacaoCheck")
    .addEventListener("change", function () {
      document.getElementById("observacao").style.display = this.checked
        ? "block"
        : "none";
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function coletarDadosDoFormulario() {
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      if (selectFormaPagamento) {
        selectFormaPagamento.addEventListener(
          "change",
  ...
  function abrirNovaAbaComJson(id) {
